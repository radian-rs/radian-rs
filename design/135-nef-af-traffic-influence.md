...
> **Status:** Phase 1 LANDED — a thin NEF (`nf-nef`, port 8009) turns an AF `Nnef_TrafficInfluence` request into a live ULCL insertion, proven end-to-end with real packets (`@nef`: the UE reaches a DNAI-named edge DN it couldn't reach a moment earlier). Later phases (PCF-mediated single-UE, group/any-UE via UDR) scoped but deferred.
...
**Phase 2 — the PCF-mediated single-UE path. Size L. (deferred)**
Extend `SmPolicyDecision` with `TrafficControlData`/`RouteToLocation`/DNAI; NEF single-UE requests go AF → NEF → PCF (`Npcf_PolicyAuthorization`, `AppSessionContext`) → the SMF's `refresh_sm_policy`, which grows an arm that turns a route-to-DNAI delta into a breakout. This is the "correct" route and the one that composes AF influence with QoS policy.
