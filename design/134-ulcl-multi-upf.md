...
> **Status:** Phases 1a + 1b LANDED — a chained `gNB → I-UPF → N9 → anchor → N6` session works end to end in both planes. §§1–2 describe the *starting* state the survey measured; §3 records what has since closed.
...
- **`session_modification_request` ignores `far_id`** — the UPF takes the *first* Update FAR with forwarding params. Latent, not live: 1b sends one Update FAR per message, each addressed to a single node, so nothing is mis-applied today. It becomes real the moment Phase 2 updates an uplink and a downlink FAR together. Fix it there.
- **Outer Header Removal is not modelled** (radian decapsulates implicitly in `gtpu::parse`). Cosmetic until a real UPF interop.

## 5. Sources

- `nf-smf/src/pdu_session.rs` (`N4Peer`, `SmfState`, `establish_chain`, `SmContext.chain`), `crates/pfcp/src/lib.rs` (`session_establishment_request`, `..._indirect_forwarding`, `session_modification_request`, `FlowFilter`/`transport_key`, `handle_n4`), `crates/n6/src/lib.rs` (`uplink`/`downlink`), `nf-upf/src/main.rs` (`serve_n3`, `serve_n6_downlink`, N6 consts), `bdd/tests/cucumber.rs` (`start_core`, the loopback-alias trick from [124](124-bdd-scripted-datapath.md)).
