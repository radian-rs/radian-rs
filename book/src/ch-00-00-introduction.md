...
Welcome to an introductory book about *radiant-rs*. radiant-rs is a greenfield
...
radiant-rs is not a fork or a port. It grows the 5G core the way the standards
...
to a forwarded IP packet through radiant-rs:
...
NF in radiant-rs is a standalone async binary — `nf-amf`, `nf-smf`, `nf-upf`, and
...
safety and an `async` runtime (`tokio`) let radiant-rs do that without a class of
...
NAS (N1), PFCP (N4), GTP-U (N3/N9) — is non-ASN.1 TLV. radiant-rs wraps mature
...
> radiant-rs is a work in progress. Where a capability is deliberately deferred
